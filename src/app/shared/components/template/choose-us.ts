...
@Component({
  ...
  template: `<div class="max-w-screen-xl mx-auto px-6 py-20">
    <div class="flex flex-col md:flex-row md:gap-6 md:items-center">
      <!-- Left: Section Title (horizontally & vertically centered) -->
      <div class="md:w-2/6 mb-6 md:mb-0 flex justify-center">
        <app-section-title-card
          [title]="langService.lang.whyChooseUs"
          [titleDesc]="
            langService.lang.ourAimIsToImproveReturnOnInvestmentsAndReduceCosts
          "
          [description]="
            langService.lang
      ...
      <div class="md:w-4/6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div *ngFor="let list of dataList" class="p-4">
...
export class ChooseUs implements OnInit {
  dataList: any;
  customClass: string =
  ...
  ngOnInit(): void {
    this.dataList = this.data.getChooseUsData().map((el: any) => {
